...
namespace HealthClinicDemo.Api.Controllers
{
    ...
    public class DoctorsController : Controller
    {
        ...
        [HttpGet]
        public async Task<IEnumerable<Doctor>> GetAsync(int pageSize = 10, int pageCount = 0)
        {
            return await _DoctorsRepository.GetAsync(Request.GetTenant(), pageSize, pageCount);
        }

        [HttpGet("speciality/{speciality}")]
        public async Task<IEnumerable<Doctor>> GetBySpecialityAsync(Specialities speciality, int count = 10)
        {
